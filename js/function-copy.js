document.addEventListener("DOMContentLoaded", function () {
    ...
    function setTranslations(translations) {
        ...
        // Definisanje mapa koje sadrže putanje do JSON fajlova za svaki jezik
        const languagePaths = {
            "sr": "js/translations_sr.json",
            "sr_cy": "js/translations_sr_cy.json",
            "ru": "js/translations_ru.json",
            "en": "js/translations_en.json"
        };

        const selectedLanguage = currentLanguage; // Ovde postavite izabrani jezik
        const selectedLanguagePath = languagePaths[selectedLanguage];

        // Učitavanje podataka iz odabranog JSON fajla
        fetch(selectedLanguagePath)
            .then(response => response.json())
            .then(translations => {


                function processCategories(translations) {
                    // Petlja za iteriranje kroz ključeve cat1, cat2, ..., cat9
                    for (let i = 1; i <= 9; i++) {
                        const categoryKey = `cat${i}`;
                        const category = translations[categoryKey];
                        ...
                        // Provjera da li postoji kategorija i ključ 'translations'
                        if (category && category.translations) {
                            // Ispis ključa 'details' za svaku kategoriju
                            console.log(category.details);

                            // Provjera da li kategorija sadrži podobjekat 'translations' i da li je niz
                            if (Array.isArray(category.translations)) {
                                // Iteracija kroz sve objekte unutar niza 'translations'
                                category.translations.forEach((translation, index) => {
                                    // Provjera da li objekat ima ključ 'hereWith'
                                    if (translation.hasOwnProperty('hereWith')) {
                                        console.log(translation.hereWith);
                                    }
                                    // Provjera da li objekat ima ključ 'drink'
                                    if (translation.hasOwnProperty('drink')) {
                                        console.log(translation.drink);
                                    }
                                    // Provjera da li objekat ima ključ 'back'
                                    if (translation.hasOwnProperty('back')) {
                                        console.log(`Ključ ${index + 1}: ${translation.back}`);
                                        // element.innerHTML = `<p>${translation.back}</p>`;
                                    } else {
                                        console.log(`Ključ 'back' nije pronađen za kategoriju ${categoryKey}, prevod ${index + 1}`);
                                    }
                                });
                            } else {
                                console.log(`Ključ 'back' nije pronađen u kategoriji ${categoryKey}`);
                            }
                        } else {
                            console.log(`Nema podataka za kategoriju ${categoryKey}`);
                        }
                    }
                }
                ...
                itemDivs.forEach((itemDiv, index) => {
                    itemDiv.addEventListener('click', function (event) {
                        ...
                        // Poziv funkcije showModal i prenos prevoda kao argument
                        showModal(itemDiv, translations);

                        stopCarousel();

                        // Ispis indeksa itemDiv-a u konzolu
                        console.log('Kliknut je itemDiv s indeksom:', index + 1);
                    });
                });


                // Funkcija za prikazivanje modalnog prozora s podacima elementa itemDiv
                function showModal(itemDiv, translations) {
                    ...
                    <div class="overlay">
                        ...
                        <div class="descriptionDiv">
                            ...
                            <div class="bottomDiv">
                                ...
                                <h2 class="reference">preporuka uz ovo:</h2>
                                <ul id="drinkList">
                                </ul>
                    ...
                    // Dodajte event listener za zatvaranje modalnog prozora
                    ...
                    processCategories(translations)
                ...
                // Nakon što su prevodi učitani, možete ih koristiti u vašoj aplikaciji
                console.log("Učitani prevodi:", translations);

                // Ovde možete koristiti učitane prevode
            })
            .catch(error => {
